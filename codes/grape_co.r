...
query="grapes"
GRAPES_co=GRAPES[which(GRAPES$sampMatbased==query),]
names(GRAPES_co)[42]="Co_occurrence"
GRAPES_co=as.data.frame(GRAPES_co[which(GRAPES_co$Co_occurrence==1),])

GRAPES_co$paramType=tolower(GRAPES_co$paramType)
GRAPES_co$sampMatType=tolower(GRAPES_co$sampMatType)
GRAPES_co$sampMatbased=tolower(GRAPES_co$sampMatbased)
GRAPES_co$sampMatCode=tolower(GRAPES_co$sampMatCode)




U_ref_co=unique(GRAPES_co$Ref)
U_type_co=unique(GRAPES_co$sampMatType)
...
for ( i in seq_along(U_ref_co)) {
  for ( j in seq_along(U_type_co)){
    ...
    tab=info_extract_co_occur(GRAPES_co,U_ref_co[i],U_type_co[j])
