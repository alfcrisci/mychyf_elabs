...
query="rice"
RICE=RICE[which(RICE$sampMatbased==query),]

RICE$paramType=tolower(RICE$paramType)
RICE$sampMatType=tolower(RICE$sampMatType)
RICE$sampMatbased=tolower(RICE$sampMatbased)


U_param=unique(RICE$paramType)
U_type=unique(RICE$sampMatType)
U_plant=unique(RICE$sampMatbased)[1]
...
for ( i in seq_along(U_param)) {
      for ( j in seq_along(U_type)){
        ...
        tab=info_extract(RICE,U_param[i],U_type[j],query)
        res_RICE_data[[z]]=tab
        res_RICE_tab[[z]]=tab$ls_tab
        res_RICE_names[[z]]=paste0(U_param[i],";",U_type[j],";",query)
        ...
        ggsave(filename = paste0(U_param[i],"_on_",U_type[j],"_mtot_simple_RICE.jpg"),dpi=300)
        ...
        ggplot(tab$mat_op, aes(Concentration_op))+
          ...
          ylab("Density")+
          ggtitle(paste(U_param[i],"on",U_type[j]),subtitle = query)
          ggsave(filename = paste0(U_param[i],"_on_",U_type[j],"_conc_simple_RICE.jpg"),dpi=300)

  if (tab$N_conc > 0) {
                    smooth_ecd_gray(as.data.frame(tab$mat_op),titleg = paste(U_param[i],"on",U_type[j]),subtitleg="rice",labelx = "Concentration_op")
                    ggsave(filename = paste0(U_param[i],"_on_",U_type[j],"_conc_full_RICE.jpg"),dpi=300)

                    }
  if (tab$N_mtot > 0) {
                    smooth_ecd_gray(as.data.frame(tab$mat_op),titleg = paste(U_param[i],"on",U_type[j]),subtitleg="rice",labelx = "meanTot_op")
                    ggsave(filename = paste0(U_param[i],"_on_",U_type[j],"_mtot_full_RICE.jpg"),dpi=300)
...
RICE_tab=data.frame(names=do.call("rbind",res_RICE_names),do.call("rbind",res_RICE_tab))
file.remove("Tabella_RICE.xls")
XLConnect::writeWorksheetToFile("Tabella_RICE.xls",RICE_tab,query)
